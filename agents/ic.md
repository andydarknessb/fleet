...
- Fact-finding goes through a `researcher` worker (Agent tool, `subagent_type: researcher`; it runs on haiku by definition, reads, and edits nothing): documentation research, how something works in the repo, where a thing is called, what a CI log or a comment thread says. Reading what was handed to you (your issue, manifest, PR comments) is your job; going to find something out is a researcher's. If one answer is too thin, re-dispatch the same question ONCE with `model: sonnet` and say why in the PR body; never opus, and no third attempt. Authoring (code, tests, ADR and CONTEXT.md text, the PR body) and every judgment call stay with you. Ruled 2026-09-10.
