...
5. **The cost report undercounts.** `measure-cycle.js:681-685` only reads sessions still in `roster.json`; rotation replaces the row, so every rotated-out dispatcher, lead and principal incarnation is invisible, and the opus `qa-reviewer` (69 runs) is a Worker that never reaches the roster: zero tokens counted. Model keys are raw strings (`sonnet` and `claude-sonnet-5` were separate tiers on 09-09).
...
| D | Stale-and-`blocked` self-heal with a guard: if `blocked`, no pending permission prompt, heartbeat older than 45 min and real work waiting, `claude respawn`, verify the pid changed, cap 2, then page. The later audit grill set the accepted threshold to 60 min; see `decisions.md`. Permission prompts stay with the existing fleet #28 handler. | The 13 h and 22.5 h outages, the manual wake recipe |
...
| H | Collector fixes before any tuning: read retired roster rows (`state/archive/roster-retired-full.jsonl`) for rotated sessions, attribute Worker transcripts to the hosting IC, normalize model keys, keep a verify-verdict history. | Six numbers in this audit rest on it |
