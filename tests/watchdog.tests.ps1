...
try {
  ...
  # fleet #136: these cases carry active Work records and no PR watcher, which the
  # watchdog now pages as watcher-stale; the watcher's own rollback flag keeps that
  # check out of every case here (tests/watcher-stale.tests.ps1 covers it).
  [IO.Directory]::CreateDirectory("$testRoot\state\flags") | Out-Null
  Write-Utf8 "$testRoot\state\flags\pr-watch-off" 'fleet #136: no PR watcher in this fixture'
