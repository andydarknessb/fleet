...
if ($tool -in @('Bash', 'PowerShell')) {
  ...
  # Heredoc bodies (a ticket written with cat, a fixture, documentation) are dropped
  # first, then the call is cut into shell segments (newline ; & && | || and the
  # parentheses of a subshell) and only a segment whose command word is gh, after
  # optional VAR=value prefixes, is a comment. A `gh issue create -b "run gh issue
  # comment ..."` is not one, and neither is a heredoc line that says so.
  $stripped = [regex]::Replace($cmd, '<<-?\s*(["'']?)(\w+)\1[^\r\n]*\r?\n[\s\S]*?\r?\n[ \t]*\2[ \t]*(?=\r?\n|$)', '<<HEREDOC_STRIPPED')
  $segments = [regex]::Split($stripped, '\r?\n|;|&&|\|\||\||&|\(|\)')
  ...
  foreach ($segment in $segments) {
    ...
    foreach ($m in [regex]::Matches($segment, '(?:--body-file|-F\s+body=@)\s*(?:"([^"]+)"|''([^'']+)''|(\S+))')) {
      ...
      if ($file -eq '-') {
        ...
        $reason = "the comment body is passed on stdin (--body-file - / body=@-), which this guard cannot inspect, so it cannot clear the comment: no fleet session may post an issue or PR comment beginning 'Approved' or 'Re-propose' (those shapes are the tenant owner's alone, since the fleet acts under the owner's own GitHub login). Write the body to a file and pass --body-file <path>; the guard reads the file's first line $cite"
        break
      }
